...
for i in range(len(useragentarray)): 
    ...
    try:
        ...
        js_file_url = "https://www.dubizzle.com.eg/.humbucker/challenge/js/validate"

        # Set up WebDriverWait with a maximum timeout of 30 seconds (you can adjust this value as needed)
        wait = WebDriverWait(driver, 30)

        # Define a custom expected condition that waits until the specified JavaScript file is loaded

        # Wait until the JavaScript file is loaded before proceeding
        script = f"return (window.performance.getEntriesByType('resource').find(e => e.name === '{js_file_url}')) !== undefined;"
        driver.execute_script(script)
    
        print("JavaScript file loaded successfully.")
        # Once the file is loaded, you can continue with your desired actions on the webpage
        cookies = driver.get_cookies() 
        for cookie in cookies:
            cookie_dict[cookie['name']] = cookie['value']
        print(cookie_dict)
    except Exception as e:
        # Handle any exceptions that may occur during the wait or interaction with the page
        print(f"Error: {e}")
...
cookie_dict.update({"x-hb-co": secret_key}) 

print("Validating secret key...")
req = Request('POST', validate_url, data=payload, )
prepped = req.prepare()
resp = s.send(prepped)
